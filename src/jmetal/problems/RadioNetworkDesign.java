...
public class RadioNetworkDesign extends Problem {
  ...
  final static int GRID_SIZE         =   82369; //Total grid size (287*287)
  final static int TRANS_NUMB          =   59; //Number of transmiters used.
  final static int TRANS_TOTAL         =   349; //Number of total transmiters.
                          //49 transmiters distributed regulary...
                          //... the rest is distributed randomly.
  final static int TRANS_TOTAL_EQUIDISTRIBUTED = 225; //Number of total transmitters when distributed uniformly
  final static int BEAMS    = 3; //Number of beams from a directive transmitter
  final static int PARAMETERS = 3;
  final static int ANTENNA_TYPES = 1; //1 type: sectorial, 2 type: omnidirectional, 3 type: square
  final static int RANGE       = 22; //Transmiter cover range (square area), when >23'6 total coverage of the terrain is achieved
  final static int RANGE_SQ      = 20;
  final static int STD_COST      = 5; //Normalized cost of antenna
  final static int COST_CIRC     = 5; //Each type of antenna has a different cost
  final static int COST_SECT     = 3;
  final static int COST_SQR      = 5;

  final static double ALPHA    = 0.5; //Value of the interferenced area relative to the uninterferenced area 
                     // (always lesser or equal to 1)

  final static double TARGET_FITNESS  = 147.85;   //Es el fitness que se ha de alcanzar para que termine 
                          //la ejecucin
  ...
  public RadioNetworkDesign(int numberOfBits){
    problemName_       = "RadioNetworkDesign" ;
  ...
  public void evaluate(Solution individual) {
     ...
     int[]  trans_location= {20,20,61,20,102,20,143,20,184,20,225,20,266,20,
                          ...
                          242,104,221,224,241,113,224,229,261,56,96,220,79,158,137,
                          180,104,147,273,262,182,205,40,174,4,69,39,230,44,115,37,31,
                          286,62,147,240,175,84,182,150,141,279,83,221,151,220,114,255,
                          81,101,231,263,20,272,150,24,55,190,255,100,18,5,131,18,68,278,
                          258,244,76,154,107,218,147,191,152,11,125,267,267,206,81,211,183,101,
  ...
  /**
   * The constraints must be normalized in the form (x) >= 0. The method
  ...
  public void evaluateConstraints(Solution  individual) {
    ...
    individual.setOverallConstraintViolation(total);
    individual.setNumberOfViolatedConstraint(violation) ;
  }

  /**
   * Determines whether the solution tested is an optimum for
